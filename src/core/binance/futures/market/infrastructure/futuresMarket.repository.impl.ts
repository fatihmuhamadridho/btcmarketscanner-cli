...
import type { FuturesExchangeInfoResponse, FuturesKlinesQuery, FuturesKlinesResponse, FuturesTicker24hrResponse } from '../domain/futuresMarket.interface';

export class FuturesMarketRepositoryImpl implements FuturesMarketRepository {
  constructor(private readonly axiosService: AxiosService) {}
  async getExchangeInfo(): Promise<FuturesExchangeInfoResponse> { return this.axiosService.get('/exchangeInfo'); }
  async getTickers24hr(): Promise<FuturesTicker24hrResponse> { return this.axiosService.get('/ticker/24hr'); }
