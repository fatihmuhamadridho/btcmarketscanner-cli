...
export function getDefaultTerminalState() {
    return {
...
export function applyTerminalCommand(input, current) {
    ...
    if (lowerCommand === 'view') {
        const nextView = arg.toLowerCase();
    ...
    if (lowerCommand === 'mode') {
        const nextMode = arg.toLowerCase();
    ...
    if (lowerCommand === 'auto') {
        ...
        const autoTrade = next === 'toggle' ? !current.autoTrade : next === 'on';
        return {
