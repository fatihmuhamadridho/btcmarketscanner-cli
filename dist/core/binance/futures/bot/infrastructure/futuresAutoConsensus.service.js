...
const executionTimeframes = ['1m', '5m', '15m', '30m', '1h', '4h'];
const INDICATOR_LOOKBACK_LIMIT = 300;
const timeframePriority = { '1m': 1, '5m': 2, '15m': 3, '30m': 4, '1h': 5, '4h': 6 };
const futuresMarketController = new FuturesMarketController();
function formatPrice(value) { return value === null ? 'n/a' : formatDecimalString(value.toFixed(2)); }
function buildSummary(interval, trend, setup) {
    return {
        ...
        takeProfitLabels: setup.takeProfits.map((takeProfit) => ({ label: takeProfit.label, valueLabel: formatPrice(takeProfit.price) })),
        trendColor: trend.color,
        trendLabel: trend.label,
    };
}
export class FuturesAutoConsensusService {
    async buildConsensus(symbol) {
        const snapshots = await Promise.all(executionTimeframes.map(async (interval) => {
            const candlesResponse = await futuresMarketController.getMarketInitialCandles(symbol, interval, INDICATOR_LOOKBACK_LIMIT);
            const candles = candlesResponse.data.map((candle) => ({ openTime: candle.openTime, open: candle.open, high: candle.high, low: candle.low, close: candle.close, volume: candle.volume, closeTime: candle.closeTime }));
        ...
        const consensusSnapshot = [...snapshots].sort((left, right) => right.setup.gradeRank - left.setup.gradeRank || timeframePriority[right.interval] - timeframePriority[left.interval])[0];
        ...
        if (consensusSnapshot) {
            ...
            if (consensusIndex >= 0)
                summaries[consensusIndex] = { ...summaries[consensusIndex], isConsensus: true };
        }
        return { consensusSetup, executionBasisLabel: executionTimeframes.join(' • ').replace('1h', '1H').replace('4h', '4H'), executionConsensusLabel, summaries, snapshots };
    }
}
